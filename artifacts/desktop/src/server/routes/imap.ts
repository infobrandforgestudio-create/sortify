...
import { db } from "../db";
import { imapConfigTable } from "../schema";
...
router.get("/config", (_req, res) => {
  ...
  res.json({ configured: true, email: config.email, imapHost: config.imapHost, imapPort: config.imapPort, username: config.username, useSsl: config.useSsl });
});
...
router.post("/config", (req, res) => {
  ...
  db.delete(imapConfigTable).run();
  db.insert(imapConfigTable).values({ email, imapHost, imapPort, username, password, useSsl, isActive: true }).run();

  res.json({ configured: true, email, imapHost, imapPort, username, useSsl });
});
